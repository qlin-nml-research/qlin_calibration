...
class EndEffectorCalibrationUI(QWidget):
    def __init__(self, robot, rosConfig, calibrationConfig):
        ...
        self._robot_config_dim = robot['robot'].get_dim_configuration_space()
    ...
    def load_ui(self):
        ...
        calibration_joint_list = rospy.get_param(os.path.join(self._calibrationConfig['calibration_data_namespace'],
                                                              "calibration_joint"), {})
        calibration_pose_list = rospy.get_param(os.path.join(self._calibrationConfig['calibration_data_namespace'],
                                                             "calibration_pose"), {})

        # generate joint display
        for i in range(1, self._robot_config_dim + 1):
            setattr(
                self, "jointLabel_" + str(i),
                QLabel(parent=self, text="joint "+str(i))
            )
            setattr(
                self, "jointDisplay_" + str(i),
                QLabel(parent=self, text="N/A")
            )
            self.gridLayout.addWidget(getattr(self, "jointDisplay_" + str(i)), i-1, 1)
            self.gridLayout.addWidget(getattr(self, "jointLabel_" + str(i)), i-1, 0)
        ...
        for ind, (key, val) in enumerate(calibration_joint_list.items()):
            ...
            self._calibration_pose_list[key] = dql.DQ(calibration_pose_list[key]).normalize()
            rospy.loginfo(
    ...
    def list_select_changed_(self, current, previous):
        if current is None:
            for ind in range(self._robot_config_dim):
                label_h = getattr(self, "jointDisplay_" + str(ind + 1))
                label_h.setText("N/A")
            return
        key = current.text()
